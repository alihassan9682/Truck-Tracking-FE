    ...
    for (const mutation of mutationsList) {
      if (mutation.type === 'attributes' && (mutation.attributeName === 'data-nav-layout' || mutation.attributeName === 'data-vertical-style')) {
        ...
        if (newValue == 'vertical') {
          let currentPath = location.pathname ? location.pathname.slice(0, -1) : location.pathname;
  ...
  return (
    <Fragment>
      <div id="responsive-overlay"
        onClick={() => { menuClose() }}
      ></div>
      <aside className="app-sidebar" id="sidebar" style={{ backgroundColor: '#111827' }} onMouseEnter={() => Onhover()}
        onMouseLeave={() => Outhover()}>

        <div className="main-sidebar-header">
          <a href={`/`} className="header-logo">
            <img src="/image-removebg-preview.png" alt="logo" className="w-20 h-auto" />
        ...
        <SimpleBar className="main-sidebar" id="sidebar-scroll">
          ...
          <nav className="main-menu-container nav nav-pills flex-column sub-open">
            <div className="slide-left" id="slide-left" onClick={() => { slideLeft(); }}></div>
         ...
         ${levelone.type === 'sub' ? 'slide has-sub' : ''} ${levelone?.active ? 'open' : ''} ${levelone?.selected ? 'active' : ''}`}
                    style={{
                      color: levelone.selected ? '#1F2937' : '#4B5563', // gray-700 for normal, dark gray for selected
                      transition: 'color 0.3s ease', // Smooth transition
                    }}
                    onMouseEnter={(e) => (e.currentTarget.style.color = '#1F2937')} // dark gray on hover
                    onMouseLeave={(e) => (e.currentTarget.style.color = levelone.selected ? '#1F2937' : '#4B5563')} // revert to normal
                  >
                    {levelone.menutitle ?
